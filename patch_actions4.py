...
pattern = re.compile(r'(\{% else %\}.*?\{\{ res\.status \}\}.*?\{% endif %\}.*?</td>.*?<td class="py-3 px-6 text-right">.*?<a href="\{\{ url_for\(\'uip_bp\.view_resolution\', org_slug=org\.slug, res_id=res\.id\) \}\}".*?>View &rarr;</a>.*?</td>)', re.DOTALL)

def replacer(match):
    return """{% elif res.status == 'DRAFT' %}
