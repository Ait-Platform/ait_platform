...
with app.app_context():
    # Only target specific slugs
    target_slugs = ['cultural_fire', 'tpx', 'hds']
    subjects = AuthSubject.query.filter(AuthSubject.slug.in_(target_slugs)).all()
        ...
        for c in active_countries:
            ...
            if not exists:
                ...
                try:
                    ...
                    db.session.rollback()
                    print(f"Skipping {c.alpha2} for {subject.slug} (already exists)")
