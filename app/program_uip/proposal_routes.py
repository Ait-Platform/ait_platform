...
def proposal_navigation():
    ...
    return {"can_use_proposals": bool(org and current_user.is_authenticated and service.appointment(org.id, current_user.id))}
...
def proposal_edit(org_slug, proposal_id=None):
    ...
    if row and (row.status != "DRAFT" or row.originator_id != current_user.id):
