...
from app.program_uip.services import audit, providers, sla, reception, documents, governance, operations, routing
...
def sla_page(org_slug):
    ...
    if request.method == "POST":
        ...
        return save()
    forms = []
    from app.program_uip.completion_routes import navigation_context
...
def decisions_page(org_slug):
    from app.program_uip.routes import _members_with_roles
    org, actor = g.organization.id, current_user.id
    audit.authorize(org, actor, governance.ADMIN)
    if request.method == "POST":
