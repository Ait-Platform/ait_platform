...
from app.program_uip.services import audit,governance,invitations,ai
...
def voting_invitations(org_slug,survey_id):
    ...
    return render_template("program_uip/invitations.html",org=org,survey=row,rows=rows,counts=counts,selected=selected,confirmation=serializer().dumps(payload))
...
def public_vote(public_org_slug,survey_id):
    ...
    response=current_app.make_response(render_template("program_uip/public_vote.html",org=org,survey=row,token=token,done=done))
...
def help_page(org_slug):
    return render_template("program_uip/help.html",org=g.organization)
...
def ai_wallet(org_slug):
    ...
    return render_template("program_uip/ai_wallet.html",org=g.organization,wallet=wallet,usage=usage,ledger=ledger,feature_usage=feature_usage,config=gateway.configuration(),can_allocate=gateway.platform_admin(actor),request_key=str(uuid.uuid4()))
...
def ai_assistant(org_slug):
    ...
    return render_template("program_uip/assistant.html",org=g.organization,tasks=ai.TASKS,result=result,request_key=str(uuid.uuid4()),config=gateway.configuration(),selected_feature=request.values.get("feature"),selected_issue=request.values.get("issue_id", ""))
