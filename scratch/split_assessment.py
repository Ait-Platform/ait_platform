...
# 1. Update the counter container ID so we can swap it entirely
...
# 2. Extract app-view-10 and replace it with two separate views (app-view-9 and app-view-10)
# Instead of complex regex for the huge block, I'll just find the start of app-view-10 and replace everything until app-view-12.

pattern = r'<div class="app-view hidden overflow-y-auto pb-20" id="app-view-10">.*?<div class="app-view hidden" id="app-view-12">'
...
<div class="app-view hidden overflow-y-auto pb-20" id="app-view-10">
  ...
  <form id="assessment-form" action="{{ url_for('sace_bp.submit_post_test') }}" method="POST">
    <!-- Hidden inputs to receive the synced checks from Slide 29 -->
...
text = re.sub(pattern, new_views, text, flags=re.DOTALL)
