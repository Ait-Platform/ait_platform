...
# Rename current client_accounts to debtors_schedule
os.rename('templates/program_mechanic/client_accounts.html', 'templates/program_mechanic/debtors_schedule.html')

# Now create a simple client_accounts.html
...
<div class="min-h-screen bg-slate-50 py-10 px-4 flex flex-col items-center">
  <div class="w-full max-w-6xl bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between border-b border-slate-100 px-6 pb-4 gap-4">
      <div>
        <h1 class="text-2xl md:text-3xl font-bold text-slate-900">Client Accounts</h1>
        <p class="mt-1 text-sm text-slate-500">Manage all your clients and view their individual ledgers.</p>
      </div>
      <div class="flex items-center gap-3">
        <a href="{{ url_for('mechanic_bp.debtors_schedule') }}" class="inline-flex items-center gap-2 rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-white hover:bg-indigo-700 transition shadow-sm">
