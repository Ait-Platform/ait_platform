...
replacement = '''            <div class="col">
                ...
                <p>Odometer: {{ "{:,.0f}".format(job_card.vehicle.mileage) ~ ' km' if job_card.vehicle.mileage else 'N/A' }}</p>
            </div>
            <div class="col">
...
content = re.sub(
    r"            <div class=\"col\">\s*<h3>Vehicle Details</h3>\s*<p><strong>\{\{ job_card\.vehicle\.registration_number \}\}</strong></p>\s*<p>Make: \{\{ job_card\.vehicle\.make \}\}</p>\s*<p>Model: \{\{ job_card\.vehicle\.model or 'N/A' \}\}</p>\s*<p>VIN: \{\{ job_card\.vehicle\.vin or 'N/A' \}\}</p>\s*</div>",
    replacement,
    content,
    flags=re.DOTALL
)
