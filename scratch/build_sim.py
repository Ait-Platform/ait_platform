...
# Replace any live poll submissions with dummy JS calls so it doesn't try to fetch()
p_views = p_views.replace("submitPoll('poll_crisis', 'FALSE', this)", "mockPoll('crisis')")
p_views = p_views.replace("submitPoll('poll_crisis', 'TRUE', this)", "mockPoll('crisis')")
p_views = p_views.replace("submitPoll('poll_root_cause', 'A', this)", "mockPoll('root')")
p_views = p_views.replace("submitPoll('poll_root_cause', 'B', this)", "mockPoll('root')")
p_views = p_views.replace("submitPoll('poll_root_cause', 'C', this)", "mockPoll('root')")
p_views = p_views.replace("submitPoll('poll_root_cause', 'D', this)", "mockPoll('root')")
p_views = p_views.replace("onclick=\"alert('Submitted!')\"", "onclick=\"mockPoll('generic')\"")
p_views = p_views.replace("submitAssessment()", "mockPoll('assessment')")

# Fix F slides - we don't want fetch() calls in F either. 
# Actually, the F buttons like "nextSlide()" were in the controller, not the slides.
...
{{% block content %}}
<div class="container mx-auto px-4 py-8 h-[90vh] flex flex-col max-w-7xl">
    
    <!-- Traffic Robot Tabs -->
    <div class="flex space-x-2 border-b-2 border-slate-700 pb-0 bg-white pt-4 px-4 rounded-t-xl shadow z-10">
    ...
    <!-- Main Content Area -->
    <div class="bg-white border border-t-0 border-slate-200 shadow-xl rounded-b-xl flex-grow overflow-hidden relative">
        
        <!-- ========================================== -->
        <!-- TAB A: GUIDE -->
        <!-- ========================================== -->
        <div id="tab-a" class="w-full h-full p-8 overflow-y-auto">
            <h1 class="text-3xl font-extrabold text-indigo-900 mb-6">SACE Auditor Guide</h1>
        ...
        <div id="tab-f" class="w-full h-full hidden flex-col relative bg-slate-900">
            ...
            <div class="bg-slate-800 p-4 border-t border-slate-700 flex justify-between items-center shadow-lg z-50">
                ...
                <div class="text-center text-slate-400 font-mono">
                    Slide <span id="f-counter">0</span> / 18
                </div>
...
<script>
    ...
    function nextFSlide() {{
        if (currentSlide < totalSlides) {{
            ...
            if ([0, 3, 4, 5, 8, 9, 10, 11, 16].includes(currentSlide)) {{
                setTimeout(() => {{
                    peekParticipant();
                }}, 1500);
            }}
    ...
    function updateSlides() {{
        ...
        if (currentSlide === 12) pIndex = 7;
        if (currentSlide === 16) pIndex = 10;
        if (currentSlide === 17 || currentSlide === 18) pIndex = 11;
