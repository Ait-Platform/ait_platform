...
class DebtorsWallet(db.Model):
    __tablename__ = 'debtors_wallet'
    id = db.Column(db.Integer, primary_key=True)
...
class DebtorsTokenTransaction(db.Model):
    __tablename__ = 'debtors_token_transaction'
    id = db.Column(db.Integer, primary_key=True)
    wallet_id = db.Column(db.Integer, db.ForeignKey('debtors_wallet.id'), nullable=False)
    amount = db.Column(db.Integer, nullable=False) # Positive for top-ups, negative for usage
    description = db.Column(db.String(255), nullable=False)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
