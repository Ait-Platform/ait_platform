...
from conftest import safe_url, baseline, migrate, migrate_phase3, migrate_phase49, current_request_schema, data as seed_data
...
def concurrent_db():
    ...
    try:
        with engine.begin() as conn:baseline(conn);migrate(conn);migrate_phase3(conn);migrate_phase49(conn);current_request_schema(conn)
