...
# I want to inject the email logic at the end of ccept_quote
# Search for:
#         db.session.commit()
#         flash("Quote accepted and moved to Confirmed Job Cards.", "success")
#     
#     return redirect(url_for('mechanic_bp.mechanic_dashboard'))

regex = r'(\s*db\.session\.commit\(\)\s*flash\("Quote accepted and moved to Confirmed Job Cards\.", "success"\)\s*)(return redirect\(url_for\(\'mechanic_bp\.mechanic_dashboard\'\)\))'
...
email_logic = '''
        ...
        if client and client.email:
            try:
                ...
                flash("Quote Confirmed and Tax Invoice emailed to client automatically!", "success")
        ...
        # Clear the old success flash that was replaced
        # We handle flashes above based on email status
