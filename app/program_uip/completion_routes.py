...
from app.program_uip.services import audit, register, providers, operations
from app.program_uip.operational_routes import page, link
...
def setup_page(org_slug):
    ...
    return render_template("program_uip/setup.html", org=g.organization)
...
def service_standards(org_slug):
    audit.authorize(g.organization.id, current_user.id, providers.STAFF)
    return render_template("program_uip/service_standards.html", org=g.organization)
