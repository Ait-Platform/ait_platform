...
old_tile1 = """<!-- Tile 1: Setup Properties -->
{% if draft_property %}
      ...
      <div class="space-y-4">
        ...
        <div class="grid grid-cols-2 gap-4">
          <div>
            ...
            <input type="number" name="bills" value="{{ draft_property.expected_bills }}" min="1" required class="w-full rounded border-2 border-slate-300 px-3 py-2 outline-none focus:border-blue-500">
          </div>
          <div>
            <label class="block text-sm font-semibold text-slate-700 mb-1">Tenants/Units</label>
            <input type="number" name="tenants" value="{{ draft_property.expected_tenants }}" min="1" required class="w-full rounded border-2 border-slate-300 px-3 py-2 outline-none focus:border-blue-500">
          </div>
        </div>
        <div class="grid grid-cols-2 gap-4">
          <div>
            <label class="block text-sm font-semibold text-slate-700 mb-1">Bulk Meter?</label>
            <select name="is_bulk" class="w-full rounded border-2 border-slate-300 px-3 py-2 outline-none focus:border-blue-500 bg-white">
              <option value="yes" {% if draft_property.is_bulk_metered %}selected{% endif %}>Yes</option>
              <option value="no" {% if not draft_property.is_bulk_metered %}selected{% endif %}>No</option>
            </select>
          </div>
          <div>
            <label class="block text-sm font-semibold text-slate-700 mb-1">Sub-Meters</label>
            <input type="number" name="sub_meters" value="{{ draft_property.expected_sub_meters }}" min="0" required class="w-full rounded border-2 border-slate-300 px-3 py-2 outline-none focus:border-blue-500">
          </div>
        </div>
...
if old_tile1 in content:
    content = content.replace(old_tile1, new_tile1)
    with open('templates/program_billing/manager_dashboard.html', 'w', encoding='utf-8') as f:
        f.write(content)
    print("Tile 1 updated in dashboard.")
else:
    print("Could not find old_tile1 pattern.")
