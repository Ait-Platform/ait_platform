...
content = re.sub(
    r"<div class=\"text-sm\">\s*<span class=\"text-slate-500 block mb-1\">Make / Model</span>.*?<span class=\"font-bold text-slate-800 font-mono\">\{\{ job_card\.vehicle\.vin or 'N/A' \}\}</span>\s*</div>\s*<div class=\"text-sm\">\s*<span class=\"text-slate-500 block mb-1\">Odometer / Mileage</span>\s*<span class=\"font-bold text-slate-800\">\{\{ job_card\.vehicle\.mileage if job_card\.vehicle\.mileage else 'N/A' \}\}</span>\s*</div>",
    replacement,
    content,
    flags=re.DOTALL
)
