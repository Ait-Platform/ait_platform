...
for name, uri in [('Local', LOCAL_URI), ('Render', RENDER_URI)]:
    ...
    try:
        ...
        with engine.begin() as conn:
            conn.execute(text('DELETE FROM user_enrollment WHERE subject_id IN (17, 18, 19, 20)'))
            res = conn.execute(text('DELETE FROM auth_subject WHERE id IN (17, 18, 19, 20)'))
