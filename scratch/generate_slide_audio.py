...
out_dir = os.path.join("app", "static", "sace_slides", "audio")
...
# Auto-generate generic scripts for slides 11 to 30 to save time for this initial pass
...
async def gen_audio():
    for s in slides:
        n = s["slide"]
        print(f"Generating audio for Slide {n}...")
        ...
        with open(concat_txt, "w", encoding="utf-8") as f:
            for file in files_to_concat:
                # ffmpeg requires forward slashes and absolute/relative paths handled carefully in concat file
                f.write(f"file '{os.path.basename(file)}'\n")
                
        final_out = os.path.join(out_dir, f"{n}.mp3")
        # Run ffmpeg
        cmd = ["ffmpeg", "-y", "-f", "concat", "-safe", "0", "-i", concat_txt, "-c", "copy", final_out]
        subprocess.run(cmd, cwd=tmp_dir, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        
        print(f"Slide {n} complete.")
