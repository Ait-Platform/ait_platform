...
# Instead of relying on index slicing which broke, let's use regex
pattern = r"<!-- Post-Pledge Registration Modal -->.*?{% endblock %}"

new_modal = '''<!-- Post-Pledge Registration Modal -->
{% if has_pledged and not current_user.is_authenticated %}
<div id="reg-modal" class="fixed inset-0 z-[70] bg-slate-900 bg-opacity-75 flex items-center justify-center p-4 backdrop-blur-sm">
...
html = re.sub(pattern, new_modal, html, flags=re.DOTALL)
