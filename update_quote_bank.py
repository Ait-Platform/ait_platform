...
# Change col width from 50% to 33.33% to fit 3 columns
...
replacement = '''            <div class="col">
                ...
                <p>Model: {{ job_card.vehicle.model or 'Unknown' }}</p>
                <p>Year: {{ job_card.vehicle.year or 'N/A' }}</p>
                <p>VIN: {{ job_card.vehicle.vin or 'N/A' }}</p>
                <p>Odometer: {{ job_card.vehicle.mileage if job_card.vehicle.mileage else 'N/A' }}</p>
            </div>
            <div class="col">
                <h3>Banking Details</h3>
                {% if shop and shop.bank_details %}
                    <p>{{ shop.bank_details | replace('\n', '<br>') | safe }}</p>
                {% else %}
                    <p>Please contact us for payment details.</p>
                {% endif %}
                <p style="margin-top: 10px;"><strong>Payment Reference:</strong><br>Job Card #{{ job_card.job_number }}</p>
            </div>'''

content = re.sub(
    r"            <div class=\"col\">\s*<h3>Vehicle Details</h3>\s*<p><strong>\{\{ job_card\.vehicle\.registration_number \}\}</strong></p>\s*<p>Make: \{\{ job_card\.vehicle\.make \}\}</p>\s*<p>Model: \{\{ job_card\.vehicle\.model or 'Unknown' \}\}</p>\s*<p>Year: \{\{ job_card\.vehicle\.year or 'N/A' \}\}</p>\s*<p>VIN: \{\{ job_card\.vehicle\.vin or 'N/A' \}\}</p>\s*<p>Odometer: \{\{ job_card\.vehicle\.mileage if job_card\.vehicle\.mileage else 'N/A' \}\}</p>\s*</div>",
    replacement,
    content,
    flags=re.DOTALL
)
