...
def debtors_router():
    ...
    if subject:
        enrollment = UserEnrollment.query.filter_by(
            user_id=user_id, subject_id=subject.id).first()
    else:
        enrollment = None
...
def dashboard():
    ...
    if not wallet:
        ...
        wallet = AitTokenWallet(user_id=current_user.id,
                                balance=bonus.bonus_tokens)
...
@login_required
def profile():
    profile_record = SoaProfile.query.filter_by(
    ...
    bank_accounts = BusinessBankAccount.query.filter_by(
        user_id=current_user.id).order_by(BusinessBankAccount.created_at.desc()).all()
    bank_form = BankAccountForm()

    sender_profiles = SenderProfile.query.filter_by(
        user_id=current_user.id).order_by(SenderProfile.created_at.desc()).all()
...
def add_sender_profile():
    ...
    if form.validate_on_submit():
        ...
        if form.is_default.data:
            SenderProfile.query.filter_by(
                user_id=current_user.id, is_default=True).update({'is_default': False})
        ...
        if logo_file:
            filename = secure_filename(
                f"debtor_{current_user.id}_{int(time.time())}_{logo_file.filename}")
            upload_folder = os.path.join(
                current_app.root_path, "static", "uploads", "debtors")
...
def set_default_sender_profile(profile_id):
    SenderProfile.query.filter_by(
        user_id=current_user.id, is_default=True).update({'is_default': False})
    sp = SenderProfile.query.filter_by(
        id=profile_id, user_id=current_user.id).first_or_404()
    sp.is_default = True
    db.session.commit()
...
def add_bank_account():
    ...
    if form.validate_on_submit():
        if form.is_default.data:
            BusinessBankAccount.query.filter_by(
                user_id=current_user.id).update({'is_default': False})
...
def set_default_bank_account(account_id):
    acc = BusinessBankAccount.query.filter_by(
...
def add_debtor():
    ...
    form.bank_account_id.choices = [(0, 'Use Default Account')] + [(
        a.id, f"{a.bank_name} - {a.account_number}") for a in bank_accounts]

    sender_profiles = SenderProfile.query.filter_by(
        user_id=current_user.id).all()
    form.sender_profile_id.choices = [(0, 'Use Default Sender Profile')] + [(
        sp.id, sp.business_name or 'Unnamed Profile') for sp in sender_profiles]
...
def edit_debtor(debtor_id):
    debtor = Debtor.query.filter_by(
        id=debtor_id, user_id=current_user.id).first_or_404()
...
def debtor_financials(debtor_id):
    ...
    has_opening_balance = DebtorLedger.query.filter_by(
        debtor_id=debtor.id, ref='OPENING').first() is not None
    all_debtors = Debtor.query.filter_by(
        user_id=current_user.id).order_by(Debtor.name.asc()).all()
...
def delete_recurring_charge(debtor_id, charge_id):
    debtor = Debtor.query.filter_by(
        id=debtor_id, user_id=current_user.id).first_or_404()
    cmap = DebtorChargeMap.query.filter_by(
        id=charge_id, debtor_id=debtor.id).first_or_404()
...
def migrate_charge_dates():
    ...
    try:
        db.session.execute(
            text("ALTER TABLE debtor_charge_map ADD COLUMN start_date DATE"))
    except Exception as e:
        pass
    try:
        db.session.execute(
            text("ALTER TABLE debtor_charge_map ADD COLUMN end_date DATE"))
    except Exception as e:
        pass
    db.session.commit()
...
def debtor_view(debtor_id):
    ...
    running_balance = 0
    all_ledgers = DebtorLedger.query.filter_by(debtor_id=debtor.id).order_by(
        DebtorLedger.txn_date, DebtorLedger.id).all()
...
def edit_transaction(txn_id):
    ...
    debtor = Debtor.query.filter_by(
        id=txn.debtor_id, user_id=current_user.id).first_or_404()
...
def generate_soa(debtor_id):
    ...
    running_balance = 0
    period_ledgers = []
    ledgers = DebtorLedger.query.filter_by(debtor_id=debtor.id).order_by(
        DebtorLedger.txn_date, DebtorLedger.id).all()
    ...
    for l in ledgers:
        ...
        if not calculated_opening_yet:
            ...
            prev_balance = running_balance - \
                l.amount if l.kind == 'debit' else running_balance + l.amount
    ...
    if request.args.get('pdf') == '1':
        try:
            pdf_bytes = html_to_pdf_bytes(
                html_content, base_url=request.host_url)
            return send_file(
            ...
            flash(
                "Error generating PDF. Please try again or print the page directly.", "danger")
            return html_content
...
def price_page():
    ...
    subject = AuthSubject.query.filter(
        db.func.lower(AuthSubject.slug) == 'debtors').first()
    if not subject:
    ...
    tariff = TokenTariff.query.filter_by(
        program_slug='debtors', action_name='soa_generation').first()
