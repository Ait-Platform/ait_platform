...
import matplotlib
matplotlib.use('Agg')  # âœ… prevents Tkinter errors
...
def _choose_tpl(ctype: str) -> str:
    ...
    candidates = [
        f"subject/loss/cards/{ctype}/index.html",
        f"subject/loss/cards/{ctype}/{ctype}.html",
        f"subject/loss/cards/{ctype}.html",
        "shared/card.html",
    ]
    ...
    for t in candidates:
        if t in available:
            return t
    return f"subject/loss/cards/{ctype}.html"
...
def _build_loss_sequence() -> list[dict]:
    """
    Exact storyboard:
      6 x instruction (welcome first if present) â†’ pause â†’ q1-25 â†’ pause â†’ q26-50 â†’ pause â†’ 8 x explain
    Falls back to synthesizing pauses/explains if the DB doesnâ€™t have them.
    ...
    # 3) Split questions into 1â€“25 and 26â€“50 by their seq ordering
    qs_sorted = qs[:]
...
def _build_loss_sequence() -> List[Dict[str, Any]]:
    ...
    # 3) Split questions into 1â€“25 and 26â€“50; synthesize if short
    first_25 = qs[:25]
    next_25  = qs[25:50]
...
def _finalize_and_redirect(user_id: int):
    ...
    This keeps your storyboard: Questions â†’ (Explains) â†’ Results.
    """
    # Prefer an explain sequence if youâ€™ve registered it
...
def _render_loss_card(name: str, **ctx):
    ...
    from jinja2 import TemplateNotFound
    candidates = [
        f"subject/loss/cards/{name}.html",
        f"subject/loss/cards/cards_{name}.html",
    ]
    last = None
    for c in candidates:
        try:
...
def _load_phase_points_for_user(user_id: int):
    """
    Compute phase totals strictly from lca_response â¨ lca_scoring_map.
...
def _start_new_run_for(uid: int) -> int:
    db.session.execute(text("""
        ...
        VALUES (:uid, CURRENT_TIMESTAMP, 'in_progress')
    """), {"uid": uid})
...
def _finish_run() -> None:
    ...
    if rid:
        db.session.execute(text("""
            UPDATE lca_run
            SET finished_at = CURRENT_TIMESTAMP, status = 'finished'
            WHERE id = :rid
        """), {"rid": int(rid)})
        db.session.commit()
...
def _persist_results_for_run(run_id: int):
    ...
    db.session.execute(text("""
        ...
        SELECT :rid, lr.user_id, :p1, :p2, :p3, :p4, CURRENT_TIMESTAMP
...
def _start_run(uid: int) -> int:
    db.session.execute(text("""
        ...
        VALUES (:uid, :subj, 'in_progress', CURRENT_TIMESTAMP)
...
def compute_loss_results(run_id: int, user_id: int | None = None) -> None:
    from app.subject_loss.services import rebuild_scorecard_for_run
    
    if user_id is None:
        uid_row = db.session.execute(text("SELECT user_id FROM lca_run WHERE id=:rid"), {"rid": run_id}).first()
        if uid_row:
            user_id = uid_row[0]
        else:
            return
    ...
    # 1. Rebuild scorecard
    rebuild_scorecard_for_run(run_id)
    
    # 2. Sum scorecard
    totals = _sum_scorecard(run_id)
    if not totals:
        return
    ...
    # 3. Upsert into lca_result using ORM
    res = db.session.query(LcaResult).filter_by(run_id=run_id).first()
    if not res:
        res = LcaResult(run_id=run_id, user_id=user_id, subject="LOSS")
        db.session.add(res)
    ...
    # Mark the UserEnrollment as completed so users aren't forced to re-take the test
    db.session.execute(text("""
        UPDATE user_enrollment 
        SET status = 'completed'
        WHERE user_id = :uid 
          AND subject_id = (SELECT id FROM auth_subject WHERE slug = 'loss' LIMIT 1)
...
def _make_pdf(html_str: str) -> bytes | None:
    """
    Generate a PDF from an HTML string if WeasyPrint is available.
    Returns PDF bytes or None if WeasyPrint is not usable.
    """
    from flask import current_app
    from app.utils.pdf_render import html_to_pdf_bytes
...
def _render_loss_pdf_bytes(run_id: int, ctx: dict) -> bytes:
    ...
    try:
        return html_to_pdf_bytes(html, base_url=request.host_url)
    except Exception as e:
        current_app.logger.error(f"PDF generation failed: {e}")
        return b""
...
def _phase_graph_data_uri_for_run(run_id: int) -> str | None:
    row = db.session.execute(text("""
        SELECT phase_1, phase_2, phase_3, phase_4,
               COALESCE(NULLIF(max_phase_1, 0), 18) AS max_phase_1,
               COALESCE(NULLIF(max_phase_2, 0), 18) AS max_phase_2,
               COALESCE(NULLIF(max_phase_3, 0), 32) AS max_phase_3,
               COALESCE(NULLIF(max_phase_4, 0), 32) AS max_phase_4
        FROM lca_result
        WHERE run_id = :rid
        ORDER BY id DESC
        LIMIT 1
...
def ensure_lca_result(run_id: int) -> LcaResult | None:
    ...
    # Compute and save results if they were missing
    compute_loss_results(run_id)
    return db.session.query(LcaResult).filter_by(run_id=run_id).first()
