...
from app.program_sms.helpers import (
    _current_sms_school,
...
def sms_entitlement_guard():
    if not current_user.is_authenticated:
        return

    now = datetime.utcnow()

    # Resolve SMS trial days from auth_subject (DB-driven)
    subj = db.session.execute(text("""
    ...
    slug = subj["slug"]
    trial_days = int(subj["trial_days"] or 0)

    ent = db.session.execute(text("""
        SELECT id, trial_end, expires_at
          FROM user_entitlement
         WHERE user_id = :uid
           AND product_slug = :slug
         LIMIT 1
    """), {"uid": int(current_user.id), "slug": slug}).mappings().first()

    # First access → create entitlement (trial if configured)
    if not ent:
    ...
    active = (
        (ent["expires_at"] and ent["expires_at"] >= now) or
        (ent["trial_end"] and ent["trial_end"] >= now)
    )

    if not active:
        return redirect(url_for("sms_bp.sms_billing"))  # add this route

    db.session.execute(text("""
        UPDATE user_entitlement
           SET last_active = :now
         WHERE id = :id
    """), {"now": now, "id": int(ent["id"])})
    db.session.commit()

@sms_bp.get("/billing", endpoint="sms_billing")
@login_required
def sms_billing():
    return render_template("subject/sms/billing.html")
...
def sms_entitlement_guard():
    ...
    if request.endpoint in {
        "sms_bp.about",
        "sms_bp.about_details",
        "sms_bp.about_comm",
        "sms_bp.start_sms",
        "sms_bp.assigned_role",
        "sms_bp.assigned_role_confirm",
        "sms_bp.assigned_role_continue",
        "sms_bp.sms_quick_register",
        "sms_bp.school_media",
    }:
        return
    ...
    # IMPORTANT: entitlement is for the OWNER/PRINCIPAL only (invited auditors/roles must not be blocked)
    school = _sms_owner_school()
    if not school:
        return
    ...
    ent = db.session.execute(text("""
        SELECT id, trial_end, expires_at, last_active
          FROM user_entitlement
         WHERE user_id = :uid
           AND product_slug = 'sms'
         LIMIT 1
    """), {"uid": int(current_user.id)}).mappings().first()

    # first ever access -> start trial
    if not ent:
        subj = AuthSubject.query.filter_by(slug="sms").first()
        trial_days = int(getattr(subj, "trial_days", 0) or 14)  # 14-day default (matches your current data)
        ...
        """), {
            "uid": int(current_user.id),
            "ts": now,
            "te": now + timedelta(days=trial_days),
            "now": now,
        })
        db.session.commit()
        return
    ...
    if not active:
        # send them to pricing/checkout for SMS (keep it simple & stable)
        next_url = request.full_path if request.full_path else request.path
        return redirect(f"/payments/quote?subject=sms&next={next_url}")

    # update last_active (throttle to reduce DB writes)
    cutoff = now - timedelta(minutes=5)
    if (ent["last_active"] is None) or (ent["last_active"] < cutoff):
        db.session.execute(text("""
            UPDATE user_entitlement
               SET last_active = :now
             WHERE id = :id
