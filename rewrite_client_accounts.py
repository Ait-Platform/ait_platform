...
if os.path.exists('templates/program_mechanic/debtors_schedule.html'):
    os.remove('templates/program_mechanic/debtors_schedule.html')
...
<div class="min-h-screen bg-slate-50 py-10 px-4 flex flex-col items-center">
  <div class="w-full max-w-6xl bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between border-b border-slate-100 px-6 pb-4 gap-4">
      <div>
        <h1 class="text-2xl md:text-3xl font-bold text-slate-900">Debtors Schedule for {{ shop.shop_name if shop else 'Your Shop' }}</h1>
          ...
          <table class="min-w-full divide-y divide-slate-200">
            <thead class="bg-slate-50">
              <tr>
                ...
                <th scope="col" class="px-6 py-3 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Balance Owed</th>
            ...
            <tbody class="bg-white divide-y divide-slate-100">
              ...
              <tr class="hover:bg-slate-50 transition">
                ...
                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                  <a href="{{ url_for('mechanic_bp.client_ledger', debtor_id=d.id) }}" class="text-white bg-indigo-600 hover:bg-indigo-700 px-3 py-1.5 rounded-md transition shadow-sm font-semibold">
                    View Ledger &rarr;
                  </a>
                </td>
              </tr>
